...
class BrainDQNMain(object):
    ...
    def train(self): # Step 1: obtain random minibatch from replay memory
        ...
        Qvalue_batch_dqn=self.Q_net(nextState_batch)
        ...
        for i in range(0, BATCH_SIZE):
            ...
            if terminal:
               ...
               #********** double dqn
                dqn_action_index = np.argmax(Qvalue_batch_dqn[i].detach().cpu().numpy())
                Q_target=QValue_batch[i][dqn_action_index]
                y_batch[i][0] = reward_batch[i] + GAMMA * Q_target
